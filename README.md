...
- `parallel.py` — **современный мульти-GPU параллелизм**: ring all-reduce (NCCL),
  ...
  **ZeRO-3/FSDP** (шардирование параметров/градиентов/Adam) — сверены с эталоном
...
- `transformer_c_backward.py` — **backward блока трансформера в C+BLAS**,
...
- `math_rl.py` — **RLVR: математика-судья**. Модель учится (a+b) mod 10 из
  проверяемой награды (GRPO + entropy-бонус); наглядно про коллапс и exploration
- `sft_rl.py` — **конвейер SFT + RL**: тёплый старт (supervised), затем RLVR —
  стабильно доходит до 0.86 (чистый RL — только ~0.57 с коллапсом)
