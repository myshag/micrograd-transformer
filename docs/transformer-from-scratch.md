...
10. [Упражнения](#глава-10-упражнения)
...
Это локальное правило: производная произведения по `a` равна `b`, по `b` равна
`a`. Множитель `out.grad` — это градиент, «пришедший сверху».

### Backward и цепное правило

Чтобы найти производную корня по всем узлам, идём от корня к листьям, применяя
**цепное правило**: если `L` зависит от `u`, а `u` от `x`, то
`dL/dx = dL/du · du/dx`.

```python
def backward(self):
    topo, visited = [], set()
    def build(v):                  # топологическая сортировка:
        if v not in visited:       # узел идёт ПОСЛЕ своих родителей
            visited.add(v)
            for child in v._prev:
                build(child)
            topo.append(v)
    build(self)
    self.grad = 1.0                # dL/dL = 1 — точка старта
    for v in reversed(topo):       # от корня к листьям
        v._backward()              # раздаём градиент по локальным правилам
```

Два тонких момента, важных до самого конца:
- **Топологический порядок** гарантирует, что к моменту обработки узла его
  собственный `grad` уже полностью накоплен.
- **`+=`, а не `=`**: один узел может входить в несколько мест графа, и вклады
  по всем путям складываются (правило производной суммы). Из-за этого перед
  каждым новым backward градиенты надо **обнулять**.
...
В классификационном варианте (ветка истории проекта) тот же трансформер,
отличая простые от составных, доходил до ~87% — и тоже за счёт правил делимости,
а не «понимания» простоты.
