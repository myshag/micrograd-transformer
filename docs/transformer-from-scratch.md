...
Любопытно, что на такой мелкой CPU-модели `torch.compile` даже медленнее eager —
JIT окупается на масштабе (большие модели, GPU, много сливаемых операций), а тут
компилировать почти нечего.
