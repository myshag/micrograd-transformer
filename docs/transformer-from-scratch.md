...
выходит — категории **расходятся** уже во ВХОДНЫХ эмбеддингах:
...
Модель никто не учил группировать слова — кластеры возникли сами из задачи
предсказания следующего токена (слова, встречающиеся в похожих контекстах,
получают похожие векторы).
...
в главе 6) и печатаем тепловой картой. Слой 3 Pythia на «The cat sat on the mat»:
...
```
  ...
  cat 0.94 0.06    .    .    .    .
  sat 0.84 0.06 0.11    .    .    .
   on 0.80 0.03 0.13 0.05    .    .
  the 0.78 0.03 0.05 0.08 0.06    .
```
...
печатью, logit lens = `final_layernorm` + `@ embed_out.T` для каждого слоя.
Ничего сверх нашего движка не нужно — данные уже все у нас.
