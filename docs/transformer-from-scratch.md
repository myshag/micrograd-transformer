...
12. [Упражнения](#глава-12-упражнения)
...
OpenBLAS, на MLP `128×512→2048→512` даёт **4.5 мс против 6.0 мс** — и даже
**обгоняет** torch eager (5.2 мс), потому что у нас нет Python-обвязки на вызов.
Вывод: наша кодогенерация не была узким местом — всё решала библиотека. `sgemm`
один, реализации разные.
